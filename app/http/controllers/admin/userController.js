...
class userController extends Controller {
    async index(req, res,next) {
        try {
            ...
            let users = await User.paginate({},{page,limit:10,sort:{createdAt:-1},populate:([{path:'roles',select:'name'}])});
            let useruse  = await User.findById(req.user.id).populate([{path:'myroles'}])
    ...
    async create(req, res,next) {
        try {
            ...
            let {name,parent,images} = req.body;
    ...
    async editCategory(req,res,next){
        try {
            ...
            await Category.findByIdAndUpdate(req.params.id,{$set:{
                name,parent : parent !== 'none' ? parent :null
                ,...objUpdate}})           
