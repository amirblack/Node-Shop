...
class Comments extends Controller {
    async index(req, res,next) {
        try {
            ...
            let Post = ({path:'Post',select:'title'})
            let comments = await Comment.paginate({approved:false},{page,limit:20,sort:{createdAt:-1},populate:Post,});
    ...
    async approved(req,res,next){
        try {
            let page  = req.query.page || 1;
            let comments = await Comment.paginate({approved:true},{page,select:'name comment Post',limit:20,sort:{createdAt:-1},
            populate:
            [{
