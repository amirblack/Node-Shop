...
class postsController extends Controller {
    async index(req, res,next) {
        try {
            ...
            let posts = await Post.paginate({},{page:page,limit:5,sort:{createdAt:-1,},select:'title viewCount commentCount type'})
            let useruse  = await User.findById(req.user.id,'name email admin roles').populate([{
                path:'myroles',
                select:'label'
            }]);
            let authorposts = await Post.paginate({user:req.user.id},{page:page,limit:5,sort:{createdAt:-1,},select:'title viewCount commentCount type'})
    ...
    async editPost(req,res,next){
        ...
        let timepost = new Date(datepicker+space.concat(timepicker)).toUTCString()
            ...
            lang,
            timepost,
            tags:this.getPostTags(req.body.tags),...objUpdate}})
