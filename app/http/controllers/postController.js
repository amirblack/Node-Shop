...
class postController extends Controller{
     ...
     async index(req,res){
        let posts = await Post.findOneAndUpdate({slug:req.params.post},{$inc:{viewCount:1}})
                                    .populate([
                                    ...
                                    .populate([
                                        {path:'comments', match:{ approved:true ,parent:null},
           ...
           let randomPost = await Post.find({categories:posts.categories.id,title:{$ne:posts.title}}).limit(5).sort({createdAt:-1}).populate('user','name').exec()
           let categories = await Category.find({},'name slug')
           let description = posts.body.split(".");
           let lastposts = await Post.find({title:{$ne:posts.title}},'title slug timepost createdAt').limit(5).sort({createdAt:-1})
    ...
    async comment(req,res,next){
        ...
        if(req.user.admin){
            newComment.set({approved:true})
        }
        else{
        newComment.set({approved:false})
        }
