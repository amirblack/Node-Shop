...
class homeController extends Controller {
    ...
    async posts(req,res){
        ...
        let posts = await Post.paginate({type:true},{page,limit:10,sort:{createdAt:-1}},'title body slug images timepost')
    ...
    async tags(req, res) {
        ...
        let tags = await Post.paginate({
            tags: req.params.tag,
            type:true,
        },{page,limit:10,sort:{createdAt:-1}}, 'title body slug images timepost')
    ...
    async categories(req, res) {
        let page  = req.query.page || 1;
        let categoryfinder = await Category.findOne({slug:req.params.category},'name images slug description')
        let category = await Post.paginate({categories:categoryfinder.id,type:true},{page,limit:10,sort:{createdAt:-1},select:'title body slug images timepost'})
        let categories = await Category.find({},'name  slug')
    ...
    async sitemap(req,res,next){
        try {
                ...
                mysitemap.add({url:'/',changefreq:'hourly',priority:1});
                mysitemap.add({url:'/post',changefreq:'hourly',priority:1});
                mysitemap.add({url:'/categories',changefreq:'daily',priority:0.7});
                mysitemap.add({url:'/tags',changefreq:'yearly',priority:0.5});
                mysitemap.add({url:'/page',changefreq:'yearly',priority:0.3});
                let posts = await Post.find({ }).sort({ createdAt : -1 }).exec();
                posts.forEach(post => {
                    mysitemap.add({ url : post.path() , changefreq : 'hourly' , priority : 1})
                })
                let categories = await Category.find({ }).sort({ createdAt : -1 }).exec();
                categories.forEach(cate => {
                    mysitemap.add({ url : cate.path() , changefreq : 'daily' , priority : 0.7})
                })
